...
class Trainer():
    def __init__(self, opt, cfg, model):
        ...
        train_dataset = object_prediction_Dataset(train_dir, resize=cfg['RESIZE'], targetresize=True, direction='top', cover_percent=0.1, randomaug=True)
        val_dataset = object_prediction_Dataset(os.path.join(cfg['DATA_DIR'], 'test'), resize=cfg['RESIZE'], targetresize=True, direction='top', cover_percent=0.1)
    ...
    def train(self, opt):
        ...
        for epoch in range(self.start_epoch, self.num_epochs) :
            ...
            for i, data in enumerate(tqdm(self.trainloader), 0):
                _, input_img, target_img = data[:3]
                b_mask = data[-2]
                target_img = torch.where(target_img>=128, 255, 0) #original segmentation target -> foreground background target
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='unet', help='object prediction model''s name for training')
    parser.add_argument('--config', type=str, default='./config/object_prediction_config.yaml', help='yaml file that has object prediction train config information')
    ...
    if opt.model == 'unet':
        model = models.Unet(in_channels=4, num_classes=2, first_outchannels=64)
