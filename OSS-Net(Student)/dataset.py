...
class blind_semantic_segmentation_Dataset(Dataset):
    ...
    def __getitem__(self, idx): #인덱싱
        ...
        return blind_image.float(), mask, b_mask, filename

class KD_semantic_segmentation_Dataset(Dataset):
    def __init__(self, data_dir, resize=512, inputresize=True, targetresize=True, transform=None, target_transform=None, direction='top', cover_percent=0.1, randomaug=None): #초기화
        self.img_dir = os.path.join(data_dir, 'input')
        self.mask_dir = os.path.join(data_dir, 'target')
        self.b_mask_dir = os.path.join(data_dir, 'b_mask')
        self.img_T_dir = os.path.join(data_dir, 'input_T')
        self.resize = resize
        self.inputresize = inputresize
        self.targetresize = targetresize
        self.images = os.listdir(self.img_dir)
        ...
        self.b_mask_groups = self._group_files(self.b_mask_dir)
        self.image_T_groups = self._group_files(self.img_T_dir)
    ...
    def __getitem__(self, idx): #인덱싱
        ...
        b_mask_path = self.b_mask_groups[group_name][idx]
        img_T_path = self.image_T_groups[group_name][idx]

        image = Image.open(img_path).convert('RGB')
        if self.inputresize: image = image.resize((self.resize, self.resize), resample=Image.BILINEAR)
        image = TF.to_tensor(image)

        image_T = Image.open(img_T_path).convert('RGB')
        if self.inputresize: image_T = image_T.resize((self.resize, self.resize), resample=Image.BILINEAR)
        image_T = TF.to_tensor(image_T)

        mask = Image.open(mask_path).convert('L')  # size : (W, H), grayscale image
        if self.targetresize: mask = mask.resize((self.resize, self.resize), resample=Image.NEAREST)

        b_mask = Image.open(b_mask_path).convert('L')  # size : (W, H), grayscale image
        if self.targetresize: b_mask = b_mask.resize((self.resize, self.resize), resample=Image.NEAREST)

        mask = np.array(mask)  # (H, W)
        mask = torch.from_numpy(mask)
        mask = mask.to(torch.int64)

        b_mask = np.array(b_mask).astype(np.uint8)  # 불러온 마스크가 255 범위라면 0/1로 변환
        b_mask = np.expand_dims(b_mask, axis=0)  # (H, W) -> (1, H, W)
        ...
        return blind_image.float(), mask, b_mask, image_T, os.path.basename(img_path)